...
public class Job  {
    ...
    public int BaseMp = 100;                // 기본 SP Max
    public int AddMp = 5;                   // 레벨별 추가 SP Max

    public int BaseAp = 10;                 // 기본 공격력
    public int AddAp = 5;                   // 레벨별 추가 AP 

    public int BaseMpRegen = 1;             // 기본 SP 리젠 
    public float BaseMpRegenInteval = 1f;   // SP 리젠 간격
    ...
    public Stat getStatInfo(int level)
    {
        ...
        stat.maxMP = BaseMp + AddMp * (level - 1);    // 맥스 Sp
        stat.attackPower = BaseAp + AddAp * (level - 1);
        stat.mpRegenerate = BaseMpRegen;
